...
3. No **SQL Editor**, execute na ordem: `001_profiles_and_check_cpf.sql`, `002_user_data.sql` e `003_pending_users.sql`.
...
**Convite para quem ainda não tem conta:** A migração `003_pending_users.sql` cria a tabela `pending_users` e as funções para convidar alguém (e-mail + CPF + nome). Em Usuários, ao adicionar/editar uma pessoa, você pode informar o e-mail e tocar em **Enviar convite**. Essa pessoa pode depois abrir o app, ir em Criar conta, informar o mesmo e-mail e CPF – o app detecta o convite e pede só a **criação da senha** para concluir o cadastro.
